...
Simulator.prototype.launchWithParams = function() {
  var args = Array.prototype.slice.call(arguments);
  var appIdentifier = args.shift();
  var cmd = `xcrun simctl launch "${this.deviceId}" "${appIdentifier}" ${args.map(s => `"${s}"`).join(' ')}`;
  return _.execPromiseGenerator(cmd);
};
